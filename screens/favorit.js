...
const Favorit = () => {
    ...
    return (
      ...
      >
        <Box p={"4"} borderBottomColor={"coolGray.300"} borderBottomWidth={1} flexDirection="row" flex={1}>
          <Box flex={1.8}>
            <Heading  lineHeight={"md"} fontSize={"md"} ><Ionicons size={20} color="#28AA9B" name="location-outline">  </Ionicons>
  ...
  return (
    <>
            ...
            return (  
              <Pressable activeOpacity={0.5} key={index} onPress={() => navigation.navigate("DetailOrder", { item: item })}>
                <Box w={"150"} mr={"4"} ml={index == 0 ? "4" : "0"}>
                  ...
                  <Ionicons size={13} color="#28AA9B"  name="location-outline"> {item.kota} </Ionicons>
                </Box>
              </Pressable>
            );
          })}
      ...
      <Heading size="lg" mt={5} mb={5} marginLeft={5} color="black">Location</Heading>
      <FlatList
        data={datas}
